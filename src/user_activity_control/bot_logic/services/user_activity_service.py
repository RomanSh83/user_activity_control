...
class UserActivityService:
    ...
    async def _send_delayed_message(self, message: Message, user: UserSchema, string_type: str) -> None:
        await asyncio.sleep(user.stand_down_delay * 60)
        await self._send_message(message=message, user=user, string_type=string_type)

    async def _send_message(self, message: Message, user: UserSchema, string_type: str) -> None:
    ...
    async def _create_stand_down_task(
        self, message: Message, user: UserSchema, string_type: str
    ) -> asyncio.Task | None:
    ...
    async def _update_last_activity(self, user_id: str, action_timestamp: datetime, task: asyncio.Task | None) -> None:
    ...
    async def proceed_activity(self, message: Message, user: UserSchema, is_command: bool = False) -> None:
        last_activity_data = self.storage.pull_data(key=user.user_id)

        # Если нужно дать ответ на команду - даем ответ и далее обрабатываем по алгоритму активность
        if is_command:
            await self._send_message(message=message, user=user, string_type=StringsTypesEnum.COMMAND.value)
        ...
        if (
            ...
            self.logger.debug(msg=f"User: {user.id}. New activity detected after timeout. Sending alert message.")
            await self._send_message(message=message, user=user, string_type=StringsTypesEnum.ALARM.value)
        ...
        # Если время меньше выдержки и есть незавершенное задание на отбой - отменяем его
            ...
            msg = f"User: {user.id}. New activity detected: current task cancelled, new task assigned."
        ...
        if not last_activity_data or last_activity_data[ActivityKeysEnum.TASK].done():
            task = await self._create_stand_down_task(
                message=message, user=user, string_type=StringsTypesEnum.STAND_DOWN.value
            )
        else:  # иначе - оставляем активную задачу
            task = last_activity_data[ActivityKeysEnum.TASK]

        # Обновляем информацию в хранилище
        await self._update_last_activity(action_timestamp=datetime.now(UTC), user_id=user.user_id, task=task)
